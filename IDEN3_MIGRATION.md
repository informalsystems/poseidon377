...
via Cauchy construction / Merlin transcripts — we need the *exact* iden3 constants to get
matching outputs. A custom generator was written instead.
...
MDS matrices for widths t=2 through t=17, generated by iden3's Grain LFSR–based parameter
derivation.

#### Parameter summary

| hash_n | Width (t) | Rate | R_F (full) | R_P (partial) | Total rounds | Round constants | Status      |
|--------|-----------|------|------------|---------------|--------------|-----------------|-------------|
| hash_1 | 2         | 1    | 8          | 56            | 64           | 128             | Done        |
| hash_2 | 3         | 2    | 8          | 57            | 65           | 195             | Done        |
| hash_3 | 4         | 3    | 8          | 56            | 64           | 256             | Done        |
| hash_4 | 5         | 4    | 8          | 60            | 68           | 340             | Done        |
| hash_5 | 6         | 5    | 8          | 60            | 68           | 408             | Done        |
| hash_6 | 7         | 6    | 8          | 63            | 71           | 497             | Done        |
| hash_7 | 8         | 7    | —          | —             | —            | —               | Not yet migrated |

All parameter sets use security level 128 and α = 5.
...
### 4. Domain separator placement

Both implementations place the domain separator in `state_words[0]` (the capacity
element) before permutation. No change was needed here — the convention already matched.

### 5. Old test vectors removed

The original Penumbra test vectors in `poseidon377/src/hash.rs` are commented out since
they are specific to the decaf377 parameterization. Correctness is validated by comparing
outputs against the iden3 JavaScript implementation.
...
- `hash_7` (t=8) parameters have not been regenerated yet.
...
- The `poseidon-paramgen` crate (Penumbra's generic parameter generator) is unused in this
  fork — the custom `gen_params.rs` example replaces it for iden3-compatible generation.
